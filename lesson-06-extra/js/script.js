...
//Если перед открытием магазина были заданы сотрудники, кнопка 'Нанять' активна
...
//и в случае что все поля пустые делаем кнопку не активной
       ...
       //обработчик на ввод данных
      rigthMenuInput[i].addEventListener('input',(e) => {
        if (canChangeField && !existNotRusAbc(e.target.value)) {
          employer_btn.disabled = false; 
        }
      });
      //обработчик на изменение
...
for (let i = 0 ; i<employers.length ; i++) {
  //добавляем обработчик события ввод данных
  employers[i].addEventListener('input',(e) =>{
  //если послдений введенный символ в поле не русский, то очищаем поле	
  if(existNotRusAbc(e.target.value)) {
  		e.target.value="";
  	}
  })
}
...
function existNotRusAbc(str) {
  if(str.match(/[\s\w\d]/i)){
	return true;	
  }
  return false;  
}
...
console.log(existNotRusAbc('fdgf'));